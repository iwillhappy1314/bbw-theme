	      	...
	      	<div class="page-content-left-inner">
	      		<div class="entry-header">
                    ...
                    <div class="flex items-center mt-2 gap-6">
                        ...
                        <?php
                            ...
                            if($companies){
                                $company_ids = wp_list_pluck($companies, 'term_id');

                                // 查询具有这些 company terms 的 stock 文章
                                $stock_posts = get_posts([
                                    'post_type' => 'stock',
                                    'post_status' => ['publish'], // 包含草稿状态
                                    'numberposts' => -1, // 获取所有匹配的文章
                                    'tax_query' => [
                                        [
                                            'taxonomy' => 'company',
                                            'field' => 'term_id',
                                ...
                                if($stock_posts){
                                    foreach ($stock_posts as $stock_post){
                                        echo '<a href="'. get_permalink($stock_post->ID) .'">' . $stock_post->post_title . '</a>';
                                    }
                                }
                            }
                        ?>
				...
				<div id="entry-content-container" class="entry-content-container">
					...
					<div class="post-tags clearfix">
                        ...
                        <?php
                            $related_companies = get_post_meta(get_the_ID(), '_related_company', true);

                            if($related_companies){
                                echo '<div class="cat-links">';
                                foreach($related_companies as $index => $related_company){
                                    $term = get_term_by('ID', $related_company, 'company');
                                    echo '<a href="'. get_term_link($related_company) .'" rel="tag">'. $term->name .'</a>';
                                    if($index !== count($related_companies) - 1){
                                        echo ', ';
                                    }
                                }
                                echo '</div>';
